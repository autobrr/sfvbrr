...
* **1.3** :x: A folder is missing `file_id.diz` (Violates `min: 1`).
...
* **2.5** :x: A folder with two `.nfo` files (Violates `max: 1`).
  ...
  Error: one or more folders had errors
  ```
...
* **3.3** :x: A folder containing two `.diz` files (Violates `max: 1`).
...
* **5.2** :white_check_mark: A folder with `course.rar`, `course.r00`, `course.r01` - `course.r99`, etc.
...
* **5.3** :x: A folder missing `.r??` volume files (Violates `min: 1`).
  ...
  Rule Validation:
    ...
    ✗ *.r?? - found 0 matches, but minimum required is 1
...
* **6.5** :x: `Sample` folder contains both `.mkv` and an `.mp4` (Violates sample file `max: 1`).
...
* **6.6** :x: `Sample` folder contains an `.rar` file (Violates pattern `.{mkv,mp4}`).
...
* **10.1** :white_check_mark: A standard album release with 12x `.mp3` files, 1x `.m3u`, 1x `.sfv`, and 1x `.nfo`.
...
* **10.3** :x: A folder with `.flac` files only.
...
* **11.1** :white_check_mark: A folder containing 2 subdirectories (e.g., "Season 1", "Season 2").
  ...
  ```bash
  Validating Release:
    Folder:       11_1/Show.Season.01.HDTV.x264-GRP
    Category:     series
